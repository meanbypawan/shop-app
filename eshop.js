...
function createHeader(searchInputData){
   ...
   headerContainer.setAttribute("class","container-fluid border border-danger");
...
function saveProductInCart(product){
   if(isLoggedIn()){
      // Perform add to cart functionnalites..
      let cartList = localStorage.getItem("cart-list");
      cartList = JSON.parse(cartList);
      
      let result =  cartList.find((obj)=>obj.id == product.id);
      if(result){
...
function viewProductDescriptionComponent(product){
   ...
   colDiv.appendChild(mainImgElement);
   let imageArray = document.createElement("div");
   for(let i=0; i<4; i++){
      let imgElement = document.createElement("img");
      imgElement.src = product.images[i];
      imgElement.setAttribute("style","height:100px; width:18%; margin-left:20px; margin-top:20px;");
      imgElement.addEventListener("click",function(){
         let temp = mainImgElement.src;
   ...
   let productDetailsCol = document.createElement("div");
   productDetailsCol.setAttribute("class","col-md-4 offset-1");
   ...
   productPrice.innerHTML = "<span class='text-warning'>"+product.price+"</span>";

   let addToCart = document.createElement("button");
   addToCart.setAttribute("class","btn btn-warning");
   addToCart.setAttribute("style","width:80%");
   addToCart.innerText = "Add To Cart";
   addToCart.addEventListener("click",function(){
      saveProductInCart(product);
   });
   productDetailsCol.appendChild(productTitle);
   productDetailsCol.appendChild(description);
   productDetailsCol.appendChild(productPrice);
   productDetailsCol.appendChild(addToCart);

   rowDiv.appendChild(productDetailsCol);
   cartContainer.appendChild(rowDiv);
